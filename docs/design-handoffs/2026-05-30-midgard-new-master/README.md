...
> **Built for Throughput. Secured by Math.**
> A Cardano-native Layer 2 optimistic rollup. High-throughput execution with a trust path that settles back to Cardano L1. Fees are paid in ADA.
...
1. **Marketing + gateway website** (`midgard-gateway`) — the only product surface in the source. A multi-page Next.js site:
   - `/` — cinematic **splash / flash-entry** ("Enter Midgard") over the world-tree image.
   - `/home` — the **Gateway**: a scroll-driven homepage with a fixed 3D/2D world-tree stage behind it, a six-layer scrollytelling sequence, and a live network HUD.
   - Content pages: `/users`, `/builders`, `/partners`, `/how-it-works`, `/security`, `/docs`, `/testnet`, `/faq`, `/official-links`.

There is no separate app / dashboard product beyond the marketing gateway and its embedded live HUD.
...
- **GitHub:** [`davedio/midgard-gateway`](https://github.com/davedio/midgard-gateway) *(private)* — Next.js 16 + React 19 + Tailwind v4. Key files:
  ...
  - `src/app/layout.tsx` — fonts (Poppins / Inter / JetBrains Mono via `next/font/google`).
  - `src/components/Gateway.tsx` — the scroll homepage, six-layer sequence, live HUD.
  - `src/components/site/ui.tsx` + `SiteNav.tsx` + `SiteFooter.tsx` — page primitives & chrome.
...
- **Mythic restraint.** The Norse framing (Midgard, world-tree, roots, "secured by math") is evocative but never campy — it's a backdrop, not a costume.
...
- **Eyebrows / kickers, chips, metrics, and CTAs are mono** — UPPERCASE for eyebrows (e.g. `TRUST ARCHITECTURE`), Title/sentence case for chips and buttons (`Build On Midgard`, `Pre-Alpha Testnet`).
...
- "Built for throughput. Secured by math." · "Cardano-native" · "trust path" · "the path can be checked" · "settles back to Cardano L1" · "Fees are paid in ADA." · "make Cardano more usable without making it *less Cardano*."
...
**Color** — Background is **Deep Ink `#07120b`** (a green-black), deepening to **Obsidian `#050c08`** at the "roots." Surfaces are **Panel `#0c1610`**. The brand green **Midgard Green `#20be43`** (and brighter **`#3be863`** for live values) signals *valid / live / active*. **Gold `#b7791f`** (bright `#e0a33c`) signals *proof / settlement / accent* and draws hairlines. **Cardano Blue `#0033ad`** (lightened to `#7fa0f0`) marks anything referencing Cardano L1. Text runs a green-tinted gray ramp: `#eaf2ec → #b7c6bc → #7c8f84 → #51635a`. Only **1–2 accent hues per screen** — green and gold do almost all the work.

**Type** — Three families, each with a strict job:
- **Poppins** (display, weight 500, tracking −0.01em) — all headings. Geometric, calm, modern.
...
All three load from Google Fonts (matching the codebase's `next/font/google`).

**Spacing & geometry** — Tight, architectural radii: **4px** (chips), **6px** (buttons), **8px** (panels). Max content width 1240px; fluid gutters `clamp(20px, 5vw, 64px)`. Sections breathe with large vertical padding (`clamp(54px, 9vh, 110px)`). Layouts are mostly single-column, left-aligned, max ~680–760px measure; the homepage alternates layer panels left/right down the scroll.
...
- **No icon font, no SVG icon library** is used in the product UI. There is no Lucide/Heroicons/Feather dependency. The visual "icons" of the interface are: the **leading gold tick** on eyebrows, the **status dots** in chips, **layer numerals** (`01`–`06`), and arrow glyphs (`→`, `↓`) typed inline in mono.
...
- **Brand marks** (in `assets/`, copied from the repo's `public/`):
  - `midgard-logo.png` — full lockup (tree-in-circle mark + "Midgard" wordmark). The wordmark is **light/white**, so the lockup only works on dark backgrounds.
...
- **`fonts/`** — see `fonts/README.md`. Fonts are Google Fonts (Poppins, Inter, JetBrains Mono) loaded via CDN; no local files are bundled.
- **`preview/`** — the Design System tab cards (type, color, spacing, component, brand specimens).
- **`ui_kits/website/`** — high-fidelity recreation of the Midgard marketing gateway: `index.html` (interactive multi-screen prototype) + JSX components. See its own `README.md`.
